...
pr_merge$type=gsub(x=pr_merge$type,pattern = "unfiltered",replacement = "Porubsky et al., 2020")
pr_merge$type=gsub(x=pr_merge$type,pattern = "filtered",replacement = "Hamadeh et al., [submitted]")

sample=data.frame(TP=c(61,80),FP=c(0,78),FN=c(10,2))
sample$precision=(sample$TP/(sample$TP+sample$FP))
sample$recall=(sample$TP/(sample$TP+sample$FN))
sample$gap=c("Chromosome-level","Chromosome-level")
sample$type = c("Hamadeh et al., [submitted]","Porubsky et al., 2020")

pr_merge=rbind(pr_merge,sample)

ggplot(pr_merge)+geom_point(aes(recall,precision,shape=gap,group=type,color=type),size=4)+
	geom_line(aes(recall,precision,group=type,color=type),size=1)+
	#geom_point(data=sample,aes(recall,precision,group=type,color=type,size=8,shape=gap))+
	xlim(0,1)+ylim(0,1)+
	labs(x="Recall",y="Precision")+
	theme_bw()+
	theme(text = element_text(size = 18),aspect.ratio = 1)+
	guides(color=guide_legend(title="Caller"),
		   shape=guide_legend(title="Max gap to SCE (bp)"),
		   size="none")+
	ggsave("Output//prec_recall.png",bg = "transparent")
